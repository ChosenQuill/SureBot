...
public class AutoMod implements MessageCreateListener, MessageEditListener {
    ...
    public AutoMod setFilter(SwearFilter filter){
        this.filter = filter;
        return this;
    }
    ...
    private void detect(Message message) {
        if (message.getAuthor().isBotUser() || message.getChannel().asServerTextChannel().filter(ServerTextChannel::isNsfw).isPresent()) {
            return;
        }
        ...
        if (message.getMentionedUsers().size() > 5) {
            ...
            String serverId = message.getServer().isPresent() ? message.getServer().get().getIdAsString() : null;
            DiscordBot.report(DUtils.createMessage("Mass Mention Report", "EmbedMessage: " + input + "User"), serverId);
        }
    }
